...
export default function App() {
  ...
  useEffect(() => {
    const fetchStories = async () => {
      try {
        const topStoriesRes = await axios.get("/api/topstories");
        console.log("Top Stories Response:", topStoriesRes.data);
        setTopStories(topStoriesRes.data.stories || []);

        const bestStoriesRes = await axios.get("/api/beststories");
        console.log("Best Stories Response:", bestStoriesRes.data);
        setBestStories(bestStoriesRes.data.stories || []);

        const newStoriesRes = await axios.get("/api/newstories");
        console.log("New Stories Response:", newStoriesRes.data);
        setNewStories(newStoriesRes.data.stories || []);
      } catch (error) {
        setError("Error fetching stories. Please try again later.");
        console.error(error);
